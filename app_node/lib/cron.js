...
module.exports = function (db, sessionMap, fs, startDEVICE) {
    ...
    const interval = setRandomInterval(()=>{
        ...
        console.log("di Mulai pada : " + date);
    ...
    //             }
    //             if (sessionMap.has(parseInt(de.nomor))) {
    //                 let sql = `SELECT * FROM pesan WHERE status='MENUNGGU JADWAL' OR status='GAGAL' AND sender = ${de.nomor} ORDER BY RAND()`;
    //                 // let sql = `SELECT * FROM pesan WHERE status='MENUNGGU JADWAL' OR status='GAGAL' AND sender = ${de.nomor} ORDER BY RAND() LIMIT ${de.chunk}`;
    //                 const wabestie = sessionMap.get(parseInt(de.nomor)).chika
    //                 db.query(sql, function (err, result) {
    ...
    //     console.log('Test Ambil data')
    //     let sql = `SELECT *  FROM blast ORDER BY RAND() LIMIT 3`;
    //     db.query(sql, function (err, result) {
