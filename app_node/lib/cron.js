...
module.exports = function (db, sessionMap, fs, startDEVICE) {
    ...
    const interval = setRandomInterval(() => {
        ...
        let sql2 = `SELECT * FROM blast WHERE status != 'terkirim' ORDER BY RAND() LIMIT 1`;
        db.query(sql2, function (err, resultss) {
            if(resultss != null){
                ...
                resultss.forEach(async dw => {
                    ...
                    switch (dw.type) {
                        case "Text":
                            console.log("coba kirim pesan text ke : " + dw.tujuan);
                            ...
                            break
                        case "Text & Media":
                            console.log("coba kirim media text ke : " + dw.tujuan);
                            const date = new Date().toLocaleString()
                            console.log("Berakhir pada : "+ date);
