...
class Dashboards extends CI_Controller
{
  ...
  public function index()
  {
    if ($this->input->post()) {
      ...
      //  Ambil data device 
      $check_limit = $this->db->get_where('device', ['pemilik' => $this->session->userdata('id_login')])->num_rows();
      //  Check limit device
      if ($check_limit >= $users->limit_device) {
        $this->session->set_flashdata('error', 'You have exceeded the device limit.');
        redirect(base_url('home'));
      }
      // Ambil data Nomor
      $check_nomor = $this->db->get_where('device', ['nomor' => $nomor])->num_rows();
      // Check Nomor
      if ($check_nomor == 0) {
  ...
  public function invoice()
  {
    ...
    $account = $this->db->get_where('account', ['id' => $this->session->userdata('id_login')])->row();
    ...
    $data = [
      ...
      //additional
      'pricelist' => $this->db->get_where('pricelist', ['id' => $account->id_pricelist])->row(),
      'pesan' => $this->db->get_where('pesan', ['status' => 'TERKIRIM', 'make_by' => $this->session->userdata('id_login'), 'status_pay' => 'belum_bayar'])->num_rows(),
      'reports' => $this->db->get_where('reports', ['status' => 'Sent', 'make_by' => $this->session->userdata('id_login'), 'status_pay' => 'belum_bayar'])->num_rows(),
      'broadcast' => $this->db->get_where('blast', ['status' => 'terkirim', 'make_by' => $this->session->userdata('id_login'), 'status_pay' => 'belum_bayar'])->num_rows()
  ...
  {
    // Variabel nomor disini untuk mengambil nomor WhatsApp untuk di jadikan sebagai Host $this->uri->segment(2) berarti ambil value url 
    // contoh url http://localhost/whatsapp/app/device/0851551050xxx
    // http://localhost/whatsapp/app/ artinya index ke 0, device index ke 1, 0851551050xxx index ke 2
    $nomor = htmlspecialchars(str_replace("'", "", $this->uri->segment(2)));
    // Jika Method POST bernilai benar maka
    if ($this->input->post()) {
      // Variabel $webhook Optional 
      $webhook = _POST('webhook');
      // Variabel $chunk Optional 
      $chunk = _POST('chunk');
      ...
      //Variabel data dari model yang di ambil dari database kemudian di return ke view
      $data = [
        'title' => 'Device',
        'row' => $query->row(),
  ...
  public function pricelist()
  {
    if ($this->input->post()) {
      ...
      // var_dump($date);
      // die;
      if ($this->input->post('type') == 'standart') {
        $datainsert = [
          'type' => 'standart',
          'nama' => 'Paket Standart (1 Bulan)',
          'price' => _POST('price'),
          'description' => _POST('description'),
          'limit_device' => _POST('limit_device'),
          'make_by' => $this->session->userdata('id_login'),
          'created_at' => $date
        ];
        $this->db->insert('pricelist', $datainsert);
      } else if ($this->input->post('type') == 'reguler') {
        $datainsert = [
          'type' => 'reguler',
          'nama' => 'Paket Reguler (6 Bulan)',
          'price' => _POST('price'),
          'description' => _POST('description'),
          'limit_device' => _POST('limit_device'),
          'make_by' => $this->session->userdata('id_login'),
        ...
        $this->db->insert('pricelist', $datainsert);
      } else if ($this->input->post('type') == 'premium') {
        $datainsert = [
          'type' => 'premium',
          'nama' => 'Paket Premium (1 Tahun)',
          'price' => _POST('price'),
          'description' => _POST('description'),
          'limit_device' => _POST('limit_device'),
          'make_by' => $this->session->userdata('id_login'),
      ...
      $this->session->set_flashdata('success', 'Successfully added Pricelist.');
      redirect(base_url('pricelist'));
    } else {
      $data = [
        'title' => 'Pricelist WA Bestie',
        'respon' => $this->db->get_where('pricelist', ['make_by' => $this->session->userdata('id_login')])
      ];
      view('pricelist', $data);
    }
  }
  ...
  public function users()
  {
    ...
    if ($this->input->post()) {
      ...
      $pricelist = $this->db->get_where('pricelist', ['id' => $id])->row();
      $date_now = date('Y-m-d H:m:s');
      $password = _POST('password');
      $lembaga = _POST('lembaga');
      $operator = _POST('operator');
      $wa_bisnis = _POST('wa_bisnis');
      $wa_operator = _POST('wa_operator');
      $email = _POST('email');
      $address = _POST('address');
      $id_pricelist = _POST('id_pricelist');
      if ($this->input->post('lifetime') == 'on') {
        $expired = null;
      } else if ($pricelist->type == 'standart') {
        $expired = date('Y-m-d', strtotime('+1 month', strtotime($date_now)));
      } else if ($pricelist->type == 'reguler') {
        $expired = date('Y-m-d', strtotime('+6 month', strtotime($date_now)));
      } else if ($pricelist->type == 'premium') {
        $expired = date('Y-m-d', strtotime('+1 year', strtotime($date_now)));
      }

      $key = md5("laa_ilaaha_illallah_" + date('H:i'));
      ...
      $this->db->insert('account', [
        ...
        'status' => 'active',
        'expired' => $expired,
        'limit_device' => $pricelist->limit_device,
        'api_key' => $key
      ]);
      ...
      $data = [
        'title' => 'Users',
        'pricelist' => $this->db->get('pricelist'),
